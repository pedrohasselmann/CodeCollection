      ...
      SUBROUTINE STELLAR_INT(vm0,r0,vl0,t0,d0,x,z,
     $  radius,vmass,d,p,t,vlum,igrad)
...
c
      ...
      real*8 vm0, r0, vl0, t0, d0, x, z
      real*8 d(210),vl(210),t(210),pg(210),eps(210),vm(210)
      real*8 radius(210),vkappa(210),pr(210),p(210)
      real*8 vmass(210), vlum(210)
      integer igrad(210)
c
...
!f2py real, intent(in) :: vm0, r0, vl0, t0, d0, x, z
!f2py real, dimension(210), intent(out) :: radius,vmass,d,p,t,vlum
!f2py integer, dimension(210), intent(out) :: igrad
...
c     gbf_t eh a razao entre o fator guilhotina (pag 160) e o fator de
c     Gaunt para absorcao ligado-livre (pag 449). Seu valor eh dado em
c     funcao de t, d e x na tabela 10-15 (pag. 509).
c
      fpp=1.04D+00
      fcn=1.33D+00
      psi=1.1D+00
...
c
      ...
      h=r0star/200.D+00
      r=r0star+h/2.D+00
c      open(1,file='pontos1.dat')
c      open(2,file='pontos2.dat')
c      open(3,file='pontos3.dat')
c      write(1,*) '       raio     massa        dens        press    
c     1temp        lumin  transp'
c      write(2,*) '   raio       pgas        prad      epspp      epscno
c     1   epstot'
c      write(3,*) '   raio      kapbf      kapff      sigmae      kaptot'
c
...
10    continue
      ...
      radius(i)=raio1/(rsun*r0)
      vmass(i)=vm(i)/(vmsun*vm0)
      vlum(i)=vl(i)/(vlsun*vl0)
...
c
      ...
      if(raio1.ge.h .AND. vl(i)/(vlsun*vl0).ge.0e0) go to 10

      return
      end subroutine STELLAR_INT
